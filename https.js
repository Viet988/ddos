...
if (cluster.isMaster) {
 ...
 if (debugMode) {
     const interval = setInterval(() => {
         try {
             // Nếu parent process đã ngắt kết nối thì dừng gửi
             if (!process.connected) {
                 console.warn("⚠️ Parent process disconnected — clearing debug interval.")
                 clearInterval(interval)
                 return
             }

             // Giữ tối đa 4 trạng thái mới nhất
             ...
             // Gửi dữ liệu về parent process
             process.send(statusesQ)

         } catch (err) {
             // Nếu pipe bị đóng, dừng gửi
             if (err.code === 'EPIPE') {
                 console.warn("⚠️ Pipe closed — stopping debug updates.")
                 clearInterval(interval)
             } else {
